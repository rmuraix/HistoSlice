...
class SlideReaderBackend(ABC):
    ...
    def mpp(self) -> tuple[float, float] | None:
        """Microns per pixel (mpp_x, mpp_y) at level 0, or None if not available."""
...
class PyVipsBackend(SlideReaderBackend):
    ...
    def __init__(self, path: str) -> None:
        ...
        # not decode full pixels until needed. Some formats (e.g. JPEG) do not
        # support the page option, so fall back gracefully.
        try:
        ...
        if n_pages > 1:
            ...
            self.__synthetic_levels = False
        else:
            # Synthesize a pyramid for flat images to match previous behavior.
            slide_h, slide_w = int(self.__img0.height), int(self.__img0.width)
            lvl = 0
            while (
                lvl == 0 or max(slide_w, slide_h) // 2**lvl >= self.MIN_LEVEL_DIMENSION
            ):
                level_dims[lvl] = (
                    slide_h // 2**lvl,
                    slide_w // 2**lvl,
    ...
    def mpp(self) -> tuple[float, float] | None:
        ...
        """
        # Try OpenSlide properties first
        try:
    ...
    # -------------------- reading APIs --------------------
    def _synthetic_page(self, level: int) -> "pyvips.Image":
        if level == 0:
            return self.__img0
        target_h, target_w = self.level_dimensions[level]
        scale_w = target_w / float(self.__img0.width)
        scale_h = target_h / float(self.__img0.height)
        page = self.__img0.resize(scale_w, vscale=scale_h, kernel="nearest")
    ...
    def _page(self, level: int) -> "pyvips.Image":
        ...
        if self.__synthetic_levels:
            return self._synthetic_page(level)
