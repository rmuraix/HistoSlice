...
def downscale_for_thumbnail(
    ...
    Ensures the number of pixels does not exceed `max_pixels` to prevent
    JPEG size limit issues and reduce disk space while maintaining visibility
    for tile annotation review.
    Uses area interpolation for downscaling; leaves image unchanged if small enough.

    Args:
        image: Input image array.
        max_pixels: Maximum number of pixels allowed. Defaults to 1,000,000
            (approximately 1000x1000 pixels).
